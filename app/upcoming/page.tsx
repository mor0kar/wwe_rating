...
import { useCustomEvents, removeCustomEvent, type CustomEvent } from '@/lib/customEvents'
import { dateParts, fmtDateShort } from '@/lib/format'
...
export default function UpcomingPage() {
  ...
  // ID des Custom-Events, dessen Entfernen-Bestätigung gerade offen ist
      ...
      >
        <div className="flex items-center gap-3 min-w-0">
          ...
          <div className={`shrink-0 text-center min-w-[52px] ${isToday ? 'text-red-400' : 'text-zinc-400'}`}>
            <p className="text-[10px] font-medium uppercase leading-none mb-0.5">{pill.weekday}</p>
            <p className="text-xl font-black leading-none">{pill.day}</p>
            <p className="text-[10px] leading-none mt-0.5">{pill.month}</p>
          </div>
        ...
        <div className="flex flex-col items-end gap-1.5 shrink-0">
          ...
          {isCustom(ev) && (
            confirmRemove === ev.id ? (
              /* Inline-Bestätigung statt window.confirm — konsistent mit dem Rest der App */
              <div className="flex items-center gap-1.5">
                <span className="text-[10px] text-zinc-400">Wirklich entfernen?</span>
                <button
                  onClick={() => setConfirmRemove(null)}
                  className="text-[10px] px-2 py-1 border border-zinc-700 rounded text-zinc-500 hover:text-zinc-300 transition-colors"
                >
                  Nein
                </button>
                <button
                  onClick={() => { removeCustomEvent(ev.id); setConfirmRemove(null) }}
                  className="text-[10px] px-2 py-1 bg-red-500 hover:bg-red-600 text-white rounded font-medium transition-colors"
                >
                  Ja
                </button>
              </div>
              ...
              <div className="flex items-center gap-2">
                <button
                  ...
                  className="text-[10px] text-zinc-500 hover:text-zinc-200 px-1.5 py-1 transition-colors"
                >
                  Bearbeiten
                </button>
                <span className="text-zinc-700 text-[10px]">·</span>
                <button
                  onClick={() => setConfirmRemove(ev.id)}
                  className="text-[10px] text-zinc-500 hover:text-red-400 px-1.5 py-1 transition-colors"
                >
                  Entfernen
                </button>
              </div>
            ...
            <ul className="mt-2 space-y-1">
              {gaps.map(g => (
                <li key={g.weekStart} className="text-xs text-amber-100">
                  <span className="text-amber-300/70">{fmtDateShort(g.weekStart)}–{fmtDateShort(g.weekEnd)}:</span>{' '}
